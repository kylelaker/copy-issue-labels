...
function createIssue(number, labels, trackedInIssues) {
    return {
        number,
        labels: {
...
function createResult(linkedIssues) {
    return {
        pullRequest: {
...
function issueWithoutTracking(issue) {
    return {
        number: issue.number,
        labels: issue.labels,
    };
...
describe("Validate parsing of Issues from the GraphQL API", () => {
    it("should return an empty list when no issue is referenced", async () => {
        expect((0, labels_1.getAllIssues)(emptyResult)).toEqual([]);
    });
    it("should return the linked issue when there is only one", async () => {
        expect((0, labels_1.getAllIssues)(oneLinkedIssueWithLabelsNotTracked)).toEqual(oneLinkedIssueWithLabelsNotTracked.pullRequest.closingIssuesReferences.nodes.map(issueWithoutTracking));
    });
    it("should return the parent and the linked issue", async () => {
        expect((0, labels_1.getAllIssues)(oneLinkedIssueWithLabelsTracked)).toEqual([
...
describe("Validate getting all labels from GraphQL response", () => {
    it("should work using a real example", async () => {
        expect((0, labels_1.getUniqueLabelsFromIssues)((0, labels_1.getAllIssues)(realExample.repository))).toEqual(["bug", "question", "javascript"]);
    });
    ...
    it.each([
        ...
        expect((0, labels_1.getUniqueLabelsFromIssues)((0, labels_1.getAllIssues)(input)).sort()).toEqual(expected.sort());
    });
});
